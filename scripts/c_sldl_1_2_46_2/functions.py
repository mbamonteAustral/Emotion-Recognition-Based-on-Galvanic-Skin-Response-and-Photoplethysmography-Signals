...
def nearest_multiple(number, multiple, print_result=False):
    """
...
def print_results_from_file(knn_perm, dt_perm, rf_perm, svm_perm, gbm_perm, bddae_perm, dummy_perm, print_results=True, show_plot=True):
    """
    ...
    def delete_nan_rows(data):
        """
        Delete rows with at least one nan value. 
        data: numpy array
        Returns numpy array without nan values. 
        """
    ...
    if print_results:
        ...
        knn_perm = delete_nan_rows(knn_perm)
        ...
        dt_perm = delete_nan_rows(dt_perm)
        ...
        rf_perm = delete_nan_rows(rf_perm)
        ...
        svm_perm = delete_nan_rows(svm_perm)
        ...
        gbm_perm = delete_nan_rows(gbm_perm)
        ...
        bddae_perm = delete_nan_rows(bddae_perm)
        ...
        dummy_perm = delete_nan_rows(dummy_perm)
...
def valence_bimodal_deep_denoising_AE(convolutionalGSR_v, convolutionalPPG_v, vx_trainGSR, vx_testGSR, vx_trainPPG, vx_testPPG, vy_train, vy_test, show_plots=0):
    ...
    xr = layers.Permute((2,1))(y)  #to concatenate one map after another.
    yf = layers.Flatten()(xr)
    d1 = layers.Dropout(rate=0.2)(yf)  #0.2 | con 0.7 anda bien también
    output = layers.Dense(1,activation="sigmoid",kernel_initializer=tf.keras.initializers.GlorotNormal(seed=12345),bias_initializer=initializers.Zeros())(d1)
