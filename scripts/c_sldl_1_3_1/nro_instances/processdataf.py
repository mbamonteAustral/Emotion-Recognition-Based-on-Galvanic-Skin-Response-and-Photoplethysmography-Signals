...
def process_data(participant):
    ...
    combined_df_mapped = map_valence_arousal(combined_df_median, scheme = 'bipartite', number_thresholds = 2, threshold = 5, L = 2, H = 8)

    X_windowed, y_median = build_dataset(combined_df_mapped)

    num_rows_with_nan = y_median.isna().any(axis=1).sum()
    print("num_rows_with_nan: ", num_rows_with_nan)

    num_rows_with_nan = y_median.isna().any(axis=0).sum()
    print("num_rows_with_nan: ", num_rows_with_nan)
